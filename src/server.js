...
const { scrapeToRSS, scrapeLinks } = require('./scraper');
const {
    initDB,
    saveArticle,
    getArticles,
            ...
            if (!siteUrl) {
                ...
                scrapeLinks(siteUrl)
                    .then((items) => {
                        const rows = items
                            .map((it) => {
                                return `<tr><td class="border px-2 py-1">${it.title}</td><td class="border px-2 py-1"><a href="${it.link}" target="_blank">${it.link}</a></td></tr>`;
                            })
                            .join('');
                        const tableHtml =
                            '<table class="min-w-full border-collapse mt-4"><thead><tr><th class="border px-2 py-1">Title</th><th class="border px-2 py-1">Link</th></tr></thead><tbody>' +
                            rows +
                            '</tbody></table>';
                        const html = scraperTemplate
                            .replace('{{url}}', siteUrl)
